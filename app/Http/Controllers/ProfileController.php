...
class ProfileController extends Controller
{
    public function index($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            return view('client.pages.profile.user.profile', ['post' => $post, 'alluser' => $alluser, 'listChat' => $listchat, 'notify' => $notify, 'allgroup' => $allgroup, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'me_you' => $me_you, 'you_me' => $you_me, 'check_friend' => $check_friend]);
        }
    }
    ...
    public function introduce($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            return view('client.pages.profile.user.introdu', ['post' => $post, 'alluser' => $alluser, 'listChat' => $listchat, 'notify' => $notify, 'allgroup' => $allgroup, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'me_you' => $me_you, 'you_me' => $you_me, 'check_friend' => $check_friend]);
        }
    }
    ...
    public function image($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            $you_me = Http::withOptions(['verify' => false])->get($url . 'Newsfeed/CheckFriend/' . $id . '/' . $_COOKIE['user'])->json();
            $check_friend = Http::withOptions(['verify' => false])->get($url . 'Newsfeed/CheckFriend/' . $_COOKIE['user'] . '/' . $id)->json();
            return view('client.pages.profile.user.img', ['post' => $post, 'alluser' => $alluser, 'listChat' => $listchat, 'notify' => $notify, 'allgroup' => $allgroup, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'me_you' => $me_you, 'you_me' => $you_me, 'check_friend' => $check_friend]);
        }
    }
    ...
    public function friend($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            $you_me = Http::withOptions(['verify' => false])->get($url . 'Newsfeed/CheckFriend/' . $id . '/' . $_COOKIE['user'])->json();
            $check_friend = Http::withOptions(['verify' => false])->get($url . 'Newsfeed/CheckFriend/' . $_COOKIE['user'] . '/' . $id)->json();
            return view('client.pages.profile.user.friend', ['post' => $post, 'alluser' => $alluser, 'listChat' => $listchat, 'notify' => $notify, 'allgroup' => $allgroup, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'me_you' => $me_you, 'you_me' => $you_me, 'check_friend' => $check_friend]);
        }
    }
}
