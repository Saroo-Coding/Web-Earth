...
class ProfileController extends Controller
{
    ...
    public function introduce($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            return view('client.pages.profile.user.introdu', ['post' => $post, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'check_friend' => $check_friend]);
        }
    }
    ...
    public function image($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            $friend = Http::withOptions(['verify' => false])->get($url . 'Account/MyFriend/' . $id)->json();
            $check_friend = Http::withOptions(['verify' => false])->get($url . 'Newsfeed/CheckFriend/' . $_COOKIE['user'] . '/' . $id)->json();
            return view('client.pages.profile.user.img', ['post' => $post, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'check_friend' => $check_friend]);
        }
    }
    ...
    public function friend($id)
    {
        if (!isset($_COOKIE['user']) || !isset($_COOKIE['token'])) {
            ...
            $friend = Http::withOptions(['verify' => false])->get($url . 'Account/MyFriend/' . $id)->json();
            $check_friend = Http::withOptions(['verify' => false])->get($url . 'Newsfeed/CheckFriend/' . $_COOKIE['user'] . '/' . $id)->json();
            return view('client.pages.profile.user.friend', ['post' => $post, 'user' => $user, 'pro_user' => $pro_user, 'friend' => $friend, 'check_friend' => $check_friend]);
        }
    }
}
