    ...
    @if (in_array($user['userId'],array_column($group['member'],'userId')))
        <div class="write-post-container">
            ...
            <div class="post-input-container">
                <textarea name="content" id="content" rows="3" placeholder="Bạn đang nghĩ gì vậy {{ $user['fullName'] }}?"></textarea>
                <img style="width: 100%;height: 100%;" id="preview_1" alt="">
                <div class="add-post-links">
                    ...
                    <p id="groupId" style="display: none">{{ $group['groupId'] }}</p>
                </div>
            </div>
        </div>
    @endif
                            ...
                            <div class="post-fas">
                                <ul>
                                    ...
                                    @if ($item['userId'] == $user['userId'])
                                        {{-- Neu admin, quan tri thi xoa dc --}}
                                        <li onclick="delete_post_group({{ $item['postId'] }})"><i class="fa-solid fa-trash-can"></i>Xóa bài viết</li>
                                    @endif
                                </ul>
                            </div>
                        </div>
        ...
        @endif --}}
            <div class="post-row">
                <div class="activity-icons">
                    @if ($item['liked'] == true)
                        <div class="like"> <i id="{{ $item['postId'] }}-heart" onclick="heart_group({{ $item['postId'] }})"
                        ...
                        <form>
                            ...
                            <div class="replys-cmt">
                                ...
                                <div class="reply-cmt">
                                    ...
                                    <div onclick="new_cmt_group({{ $item['postId'] }})" class="post-btn">Post</div>
                                </div>
                            </div>
                        </form>
                    </div>
    ...
    if(input != null){
        ...
        function new_post_group() {
            ...
            if(document.getElementById('file').files[0] != null){
                ...
                const refStorage = sRef(storage,"Group.No." + groupId+ "/" + imgName);
                ...
                UploadTask.on('state_changed',function(snapshot){
                    ...
                    $.ajax({
                        type: "POST",
                        url: url + "Groups/NewGroupPost",
                        contentType: "application/json;charset=utf-8",
                        headers: { Authorization: 'Bearer ' + cookie.token },
                        data: JSON.stringify({groupId: groupId, content: content, userId: cookie.user, img1:downloadURL,img2:'Khong',img3:'Khong'}),
                ...
                $.ajax({
                    ...
                    contentType: "application/json;charset=utf-8",
                    headers: { Authorization: 'Bearer ' + cookie.token },
                    data: JSON.stringify({groupId: groupId, content: content, userId: cookie.user, img1:'Khong',img2:'Khong',img3:'Khong'}),
                    success: function () {location.reload();},
                    error: function () { alert("Có gì đó sai sai !!!");location.reload();}
                });
            }
        }
        document.getElementById('post_button').onclick = new_post_group;
    }
</script>
@endsection
